...
and informs them when to enable or disable their probes. When a probe
fires, the OpenDTrace kernel module is responsible for executing the
necessary instrumentation code and providing the data to any
consumers.
...
system, (see Table XXX), but developers are free to create their own to
expose more or different information from their code. Providers can be
developed either for the kernel, in which case they are defined as
kernel modules, or for user space, as part of the User Defined Static
Tracing (USDT) system.
...
reached. The probe gathers data of interest and passed data back into
...
The user space interface to DTrace is the DTrace command line utility,
\texttt{drace(1)}. The \texttt{dtrace} command line utility handles
...
Figure~\ref{fig:lifecycle}. For the purposes of this example it is
assumed that the DTrace kernel module has already been loaded during
system boot. We ignore execution of code withing any of the providers
and only discuss the interactions between components. Internal
functions of interest within the kernel module and CLI are shown.
...
provider to enumerate all of its available probes and the probes are
...
to initialize some tracing state and set up memory buffers to stored
the trace data.
...
If the script did not supply any type information the compilation will
complete and any mismatch will result in a runtime error.
...
When a code execution reaches a point that has an enabled probe, the
