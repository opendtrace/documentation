This document provides an overview of how \texttt{libdtrace} processes the
...
Note that the length of the data for the trace record is not specified by the
...
\begin{figure}[!htp]
	...
	\end{tikzpicture}
	\caption{Overview of the data structures used to interpret the DTrace trace buffer}
	\label{fig:overview}
\end{figure}
...
required to process the DTrace trace buffer including metadata describing the
enabling, a pointer to the copy of the trace buffer and formatting information
(such as the flow prefix and indent---used when the DTrace is invoked with the
...
trace records}. 
	...
	\item The \texttt{dtepd\_size} field specifies the size of the DTrace trace
buffer (a pointer to the trace buffer is held in the
...
\texttt{struct~\textit{dtrace\_probedesc}} is returned from the kernel to user
space by invoking the \texttt{PROBES} ioctl.
...
DTRACE\_PROVNAMELEN (nominally 64 characters).
...
DTRACE\_MODNAMELEN (nominally 64 characters).
...
DTRACE\_FUNCNAMELEN (nominally 192 characters).
...
DTRACE\_NAMELEN (nominally 64 characters).
...
an individual trace record within the trace buffer. Each DTrace action produces
...
\subsection{DTrace aggregation trace buffer}
\label{subsec:agg_trace_buffer}

Figure~\ref{fig:agg_trace_buffer} presents an overview of a DTrace aggregation
trace buffer:
...
\begin{figure}[!h]
	...
	\end{tikzpicture}
	\caption{DTrace aggregation trace buffer.}
...
contain the compile time indetifier for the aggregation. This is determined by
...
compiler assigned identifier and user assigned name of the aggregation to be
determined.
...
aggregations are assigned \texttt{DTRACE\_AGGVARIDNONE}\footnote{Note as the
aggregation name can't be determined it cannot be included in the printed
output.}
...
The \texttt{struct \textit{dtrace\_aggbuffer}} (Figure~\ref{fig:dtagd})
specifies the metadata for an aggregation. The data structure is used solely
within the kernel\footnote{The comments within the code suggest that the
...
implementation the hash table accounts for approximately 1/8 of the
...
in the DTrace aggregation buffer where new aggregation keys are allocated;
...
The relationships between the \texttt{dtagb\_free} and \texttt{dtagb\_hash}
fields and the DTrace aggregation buffer are shown in
