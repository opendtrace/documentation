...
Furthermore, a modified version of the Register Format is used when loading and storing to array variables in DTrace. The main difference between the regular Register Format and the modified one used for arrays, is that the \registerop{r1} register location is used as the variable identificator, and the \registerop{r2} register location is used as an optional index in the array.
