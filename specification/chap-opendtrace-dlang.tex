...
(\texttt{count()}, \texttt{min()}, \texttt{max()}, \texttt{avg()}, and
\texttt{sum()}) are simple enough that pseudo-code can be supplied
within Table~\ref{tab:agg-func} while the next three functions:
\texttt{stddev()}, \texttt{quantize()} and \texttt{lquantize()},
should be understood in their mathematical expression.

The \texttt{llquantize()} function is specific to OpenDTrace, and was
written by Bryan Cantrill while at Joyent.  The purpose of
\texttt{llquantize()} is to aggregate data logarithmically over a
specified range of magnitudes, but use a frequency distribution within
each of the magnitude.
